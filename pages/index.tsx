  ...
  return (
    <Layout>
      ...
      <section style={{ marginBottom: '3rem' }}>
        <h1 style={{ marginTop: 0, fontSize: '2.5rem' }}>Welcome to My Blog</h1>
        <p style={{ fontSize: '1.15rem', color: '#adb5bd', lineHeight: 1.7 }}>
        ...
        <div style={{ display: 'flex', gap: '1rem', marginTop: '1.5rem' }}>
          ...
          <Link
            href='/contact'
            style={{ border: '1px solid #444', padding: '0.6rem 1.4rem', borderRadius: 6, color: '#fff' }}
          >
            Get in touch
          </Link>
        </div>
        ...
        <section>
          ...
          <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
            {recentPosts.map((post) => (
              ...
              >
                <Link href={`/posts/${post.slug}`} style={{ fontWeight: 600 }}>
                  {post.title}
                </Link>
                {post.date && (
